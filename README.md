...
```bash
git clone https://github.com/your-org/ai-db-client.git
cd ai-db-client
npm install
npm run build
```
...
Install it through your Codex plugin workflow or a local marketplace entry that points to this repository.
...
```json
{
  "mcpServers": {
    "ai-db-client": {
      "command": "node",
      "args": ["/absolute/path/to/ai-db-client/dist/index.js"],
      "env": {
