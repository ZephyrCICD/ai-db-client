...
function connectionConfig(context: ResolvedContext): pg.ClientConfig {
  ...
  return {
    ...
    ssl: profile.ssl as pg.ClientConfig["ssl"],
  };
}

async function withClient<T>(context: ResolvedContext, fn: (client: pg.Client) => Promise<T>): Promise<T> {
...
function pgEscapeIdentifier(value: string): string {
  return `"${value.replace(/"/g, '""')}"`;
}

function toQueryResult(context: ResolvedContext, result: pg.QueryResult, limit?: number): QueryResult {
