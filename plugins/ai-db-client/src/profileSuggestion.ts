...
function detectType(text: string): DbType | undefined {
  const lower = text.toLowerCase();
  if (lower.includes("postgresql://") || lower.includes("postgres://") || lower.includes("postgres")) return "postgres";
  if (lower.includes("mysql://") || lower.includes("mysql")) return "mysql";
  if (lower.includes("tdengine") || lower.includes("taos")) return "tdengine";
  return undefined;
}

function detectPort(text: string, type?: DbType): number | undefined {
  const explicit = text.match(/\b(?:port|p)\s*[:=]\s*(\d{2,5})\b/i)?.[1];
  if (explicit) return Number(explicit);
  if (type === "postgres") return 5432;
  if (type === "mysql") return 3306;
  if (type === "tdengine") return 6030;
  return undefined;
}
